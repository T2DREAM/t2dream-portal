...
@view_config(route_name='getAnnotation', request_method='POST', renderer='json', permission=Allow)
def t2dkp(context, request):
    ...
    URL = 'https://www.diabetesepigenome.org/annotation_metadata/'
    URL1 = '%s/annotation_metadata.json' % (urlencode(payload,doseq=True))
...
if __name__ == '__main__':
    ...
    config.add_view(getAnnotation, route_name='getAnnotation')
    config.scan()
