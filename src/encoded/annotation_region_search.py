...
def includeme(config):
    config.add_route('annotation_region_search', '/annotation_region_search{slash:/?}')
    config.add_route('suggests', '/suggests{slash:/?}')
...
@view_config(route_name='annotation_region_search', request_method='GET', permission='search')
def annotation_region_search(context, request):
    """
    Search files by region.
    """
    types = request.registry[TYPES]
    result = {
        '@id': '/annotation_region_search/' + ('?' + request.query_string.split('&referrer')[0] if request.query_string else ''),
        '@type': ['annotation_region_search'],
...
@view_config(route_name='suggests', request_method='GET', permission='search')
