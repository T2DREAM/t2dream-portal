...
class VisCollections(object):
    ...
    def __init__(self, request, vis_datasets=None, accessions=None, assembly=None):
        ...
        if self.host is None or self.host.find("localhost") > -1:
            self.host = "https://www.diabetesepigenome.org"
    ...
    def prepend_assay_labels(self, prefix):
        ...
        for vis_type in self.vis_by_types.keys():
            ...
            if vis_by_type:  # could be {}
                ...
                if vis_by_type['shortLabel'].startswith('DGA '):
                    vis_by_type['shortLabel'] = "DGA %s %s" % (prefix, vis_by_type['shortLabel'].split(None,1)[1])
                else:
...
class VisDataset(object):
        ...
        for view_tag in self.vis_dataset["view"].get("group_order", []):
            ...
            for a_file in files:
                ...
                metadata_pairs['file&#32;download'] = ( \
                    '"<a href=\'%s%s\' title=\'Download this file from the DGA portal\'>%s</a>"' %
                        ...
                        for (subgroup_tag, subgroup) in subgroups.items():
                            ...
                            if "url" in subgroup:
                                metadata_pairs[group_title] = ( \
                                    '"<a href=\'%s/%s/\' TARGET=\'_blank\' title=\'%s details at the DGA portal\'>%s</a>"' %
    ...
    def gather_pipeline_details(self):
        ...
        if software_files:
            ...
            path = '/search/?%s&software.lab!=/labs/dga-processing-pipeline/&frame=embedded' % (urlencode(params, True))
...
def get_hub(label, comment=None, name=None):
    ...
    if comment is None:
        comment = "Generated by the DGA portal"
    hub = OrderedDict([
        ('email', 't2depigenome.help@gmail.com'),
        ('genomesFile', 'genomes.txt'),
        ('longLabel', 'DGA Data Coordination Center Data Hub'),
...
def generate_batch_hubs(context, request):
    ...
    if (suffix == 'txt' and page == 'trackDb') or \
        ...
        data_policy = ('<br /><a href="https://www.diabetesepigenome.org/">'
                       'DGA data use policy</p>')
