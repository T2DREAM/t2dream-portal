...
#hashmap of annotation and corresponding file types that are being indexed
_INDEXED_DATA1 = {
    'chromatin state': {
...
#index route into configuration, this pyramid method 
def includeme(config):
    config.add_route('index_file', '/index_file')
    config.scan(__name__)


#both annotation and assay are tab delimited
def tsvreader(file):
...
#number of shards for elastic search
def index_settings():
    return {
        'index': {
            'number_of_shards': 1
